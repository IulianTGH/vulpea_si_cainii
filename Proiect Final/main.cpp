...
void drawBackground() {
    ...
    try {
        readimagefile("background.bmp", 0, 0, screen_width-1, screen_height-1);
    } catch (...) {
        // If image loading fails, draw a colored background as fallback
        setfillstyle(SOLID_FILL, BLACK);
        bar(0, 0, screen_width, screen_height);

        // Optionally print error message
        setcolor(WHITE);
        outtextxy(10, 10, "Background image failed to load");
    }
}
...
int main() {
    ...
    // Afișează fundalul imediat după inițializarea ferestrei grafice
    drawBackground();
    ...
    while (1) {
        ...
        // Afișează fundalul înainte de a desena meniul principal
        drawBackground();
        ...
        // Afișează fundalul înainte de a desena conținutul ferestrei de joc
        drawBackground();
        ...
        switch (newGame) {
            case KEY_0: // New Game
                ...
                GameBoard[0][3] = FOX;
                cleardevice();
                drawBackground(); // Afișează fundalul înainte de a desena tabla
                drawBoard();
                break;
        ...
        while (1) {
            ...
            if(winState!=0) {
                cleardevice();
                drawBackground(); // Afișează fundalul înainte de a afișa mesajul de victorie
                char text[100];
                if(winState==FOX)  {
                    setcolor(WHITE);
            ...
            if (kbhit()) {
                ...
                if (key == KEY_ESC) {
                    cleardevice();
                    drawBackground(); // Afișează fundalul înainte de a afișa meniul de salvare
